...
from cocos.prompts import make_fewshot, make_prompt
from cocos.rewards import get_reward, calculate_improve
...
def main(args):
    ...
    for batch in tqdm(data_loader):
        ...
        for num_turn in range(1, args.num_turns + 1):
            ...
            outputs = tokenizer.batch_decode(
                outputs[:, prompts['input_ids'].shape[-1]:],
                skip_special_tokens=True, clean_up_tokenization_spaces=True,
            )

            test_lists = [data['test_list'] for data in batch]
            _, corrects, responses_for_prompt = get_reward(
                None, outputs, test_lists, num_turn, corrects, responses_for_prompt,
            )
        ...
        for i, data in enumerate(batch):
            ...
            for j in range(args.num_turns):
                ...
                if j > 0:
                    ...
                    result, inco_co, co_inco, inco_inco, co_co = calculate_improve(
                        prev=corrects[i][j-1], curr=corrects[i][j],
                    )
                    if result == "better":
    ...
    for key in keys:
        avg_results[key] = round(sum(d[key] for d in eval_total) / len(test_dataset), 3)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Evaluate CoCoS model")
